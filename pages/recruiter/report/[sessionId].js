...
export default function RecruiterReport() {
  ...
  useEffect(() => {
    ...
    if (!sessionId || !secret) { setError('Missing session ID or secret.'); setLoading(false); return; }
  ...
  const downloadPDF = useCallback(async () => {
    ...
    try {
      ...
      const W = 210; const margin = 20; const col = W - margin * 2; let y = 20;

      doc.setFillColor(15, 15, 25); doc.rect(0, 0, W, 30, 'F');
      ...
      doc.setTextColor(30, 30, 40); doc.setFontSize(18); doc.setFont('helvetica', 'bold');
      doc.text(report.candidateName || 'Candidate', margin, y); y += 7;
      doc.setFontSize(10); doc.setFont('helvetica', 'normal'); doc.setTextColor(100, 100, 120);
      doc.text(`${report.subject || ''} · Completed ${report.completedAt ? new Date(report.completedAt).toLocaleDateString() : ''}`, margin, y); y += 10;

      doc.setFillColor(245, 245, 250); doc.roundedRect(margin, y, col, 22, 4, 4, 'F');
      const pills = [
      ...
      if (report.summary) {
        ...
        doc.text(summaryLines, margin + 5, y + 7); y += summaryH + 8;
      }

      doc.setFontSize(9); doc.setFont('helvetica', 'bold'); doc.setTextColor(120, 120, 140);
      doc.text('DIMENSION SCORES', margin, y); y += 5;
      const dims = [
      ...
      dims.forEach(dim => {
        const d = report.scores?.[dim.key]; if (!d) return;
        const score = d.score || 0;
        doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(30, 30, 40);
        doc.text(dim.label, margin, y + 4);
        doc.setFontSize(8); doc.setTextColor(140, 140, 160); doc.text(dim.weight, margin + 60, y + 4);
        doc.setFillColor(220, 220, 230); doc.roundedRect(margin + 75, y, 80, 4, 2, 2, 'F');
        const barColor = score >= 8 ? [34,197,94] : score >= 6 ? [108,99,255] : score >= 4 ? [245,158,11] : [239,68,68];
        doc.setFillColor(...barColor); doc.roundedRect(margin + 75, y, (score / 10) * 80, 4, 2, 2, 'F');
        ...
        if (d.evidence) {
          ...
          doc.text(`"${d.evidence}"`, margin, y + 9); y += 14;
        } else { y += 10; }
      }); y += 4;

      if (report.feedback?.length) {
        doc.setFontSize(9); doc.setFont('helvetica', 'bold'); doc.setTextColor(120, 120, 140);
        doc.text('RECRUITER NOTES', margin, y); y += 6;
        report.feedback.forEach(f => {
          ...
          doc.text(lines[0], margin + 22, y + 9); y += 16;
        });
      }
    ...
    <>
      ...
      <div style={styles.page}>
        ...
        <div style={styles.content}>
          ...
          {report && (
            <ScoreCard report={report} candidate={report.candidateName || 'Candidate'} subject={report.subject || ''} />
          )}
          ...
          <p style={styles.footer}>This report is confidential. Do not share with the candidate.</p>
        </div>
      </div>
    </>
  );
...
const styles = {
  page: {
    ...
    backgroundSize: '28px 28px', padding: '40px 24px', position: 'relative',
  ...
  content:     { position: 'relative', zIndex: 2, maxWidth: '680px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '24px' },
  header:      { display: 'flex', alignItems: 'center', justifyContent: 'space-between' },
  brandRow:    { display: 'flex', alignItems: 'center', gap: '8px' },
  brand:       { fontSize: '15px', fontWeight: 500, color: '#1a1a2e', letterSpacing: '-0.2px' },
  tag:         { fontSize: '12px', color: '#888', background: 'rgba(255,252,245,0.9)', border: '1px solid rgba(0,0,0,0.1)', padding: '4px 12px', borderRadius: '100px' },
  msg:         { fontSize: '14px', color: '#666' },
  errorCard:   { background: 'rgba(255,252,245,0.93)', border: '0.5px solid rgba(0,0,0,0.11)', borderRadius: '18px', padding: '40px', display: 'flex', flexDirection: 'column', gap: '12px' },
  downloadWrap:{ display: 'flex', justifyContent: 'center', paddingTop: '8px' },
  downloadBtn: { background: '#1a1a2e', color: '#ffffff', border: 'none', borderRadius: '10px', padding: '12px 36px', fontSize: '14px', fontWeight: 500, cursor: 'pointer', letterSpacing: '0.2px', fontFamily: 'inherit' },
  downloadBtnDisabled: { opacity: 0.5, cursor: 'not-allowed' },
  footer:      { textAlign: 'center', fontSize: '12px', color: '#bbb', paddingBottom: '40px' },
};